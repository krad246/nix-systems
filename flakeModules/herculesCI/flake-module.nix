  ...
  flake.effects = withSystem "x86_64-linux" ({hci-effects, ...}: {
    ...
    fortress-deploy = hci-effects.runNixOS {
      ssh = {
        ...
        sshOptions = "-vvv -i /etc/ssh/ssh_host_ed25519_key -o StrictHostKeyChecking=accept-new -oSetEnv=PATH=/nix/var/nix/profiles/default/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin";
      };
