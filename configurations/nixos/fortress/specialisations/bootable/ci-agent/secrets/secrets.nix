let
  fortress = {
    krad246 = {
      rsa = "ssh-rsa  krad246@fortress";
    };

    root = {
      rsa = "ssh-rsa  root@fortress";
    };
  };
  inherit (fortress) krad246 root;
in {
  "./binary-caches.age".publicKeys = [krad246.rsa root.rsa];
  "./cachix.age".publicKeys = [krad246.rsa root.rsa];
  "./cluster-join-token.age".publicKeys = [krad246.rsa root.rsa];
  "./id_ed25519_priv.age".publicKeys = [krad246.rsa root.rsa];
}
